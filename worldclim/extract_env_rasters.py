...
raster_names = bioclimatic_raster_names
...
class Raster(object):
    ...
    def __init__(self, path, size=36, nan=np.nan, out_of_bounds="error"):
...
class PatchExtractor(object):
    ...
    def __init__(self, root_path, name_extension="wc2.1_30s", size=256):
        """Constructor
    ...
    def add_all_rasters(self, **kwargs):
        ...
        for raster_name in raster_names:
            self.append(f"{raster_name}", **kwargs)
    ...
    def append(self, raster_name, **kwargs):
        ...
        r_us = Raster(self.root_path / raster_name, size=self.size,  **kwargs)
...
DATA_PATH = Path("/Volumes/My Passport/South_Africa/worldclim")

extractor = PatchExtractor(DATA_PATH, size = 6)
extractor.add_all_rasters()
print("Number of rasters: {}".format(len(extractor)))

if __name__=="__main__":
    df = pd.read_csv("hotspots_za_counts_merged_1km_trimmed.csv") 
    for index, row in df.iterrows():
        ...
        try : 
            ...
            np.save("/Volumes/My Passport/South_Africa/environmental/" + row.hotspot_id + ".npy", val)
        except :
            i+= 1
            print(row.hotspot_id, row.index) 
            pass